...
CHECKLIST = [
    ...
    ("Await admissions decision","The team will review your file and be in touch. Contact them directly with any questions.",      "https://www.brewstermadrid.com/admissions/contact-us"),
]

NEWS = [
    ("Mar 2026", "Life as an Expat Student in Madrid: How Brewster Madrid Fosters Global Citizenship", "https://www.brewstermadrid.com/news/life-as-an-expat-student-in-madrid-how-brewster-madrid-fosters-global-citizenship"),
    ("Feb 2026", "How to Choose an International School in Madrid: A Framework for Expat Families",    "https://www.brewstermadrid.com/news/how-to-choose-an-international-school-in-madrid-a-framework-for-expat-families"),
    ("Jan 2026", "Brewster Announces Matthew Colburn as Executive Director of BA International",        "https://www.brewstermadrid.com/news/brewster-announces-matthew-colburn-as-executive-director-of-ba-international"),
]
...
with st.sidebar:
    ...
    c1, c2 = st.columns(2)
    c1.metric("👍", thumbs_up)
    c2.metric("👎", thumbs_down)

    csv_data = get_csv_download()
    if csv_data:
        st.download_button("⬇️ Download Chat Log (CSV)", data=csv_data,
                           file_name=f"brewster_chat_{datetime.date.today()}.csv",
                           mime="text/csv", use_container_width=True)

    st.markdown("---")
    st.markdown("<small>Powered by Claude AI · [brewstermadrid.com](https://www.brewstermadrid.com)</small>",
                unsafe_allow_html=True)
...
with tab_chat:
    ...
    for col, (num, lbl) in zip(cols, STATS):
        col.markdown(f'<div class="stat-box"><div class="stat-num">{num}</div><div class="stat-lbl">{lbl}</div></div>',
                     unsafe_allow_html=True)
    ...
    for idx, msg in enumerate(st.session_state.messages):
        with st.chat_message(msg["role"], avatar="🎓" if msg["role"] == "assistant" else "🙋"):
            ...
            if msg["role"] == "assistant":
                ...
                if c1.button("👍", key=f"up_{idx}", type="primary" if current_fb == "👍" else "secondary"):
                    st.session_state.feedback_map[fb_key] = "👍"
                    st.rerun()
                if c2.button("👎", key=f"dn_{idx}", type="primary" if current_fb == "👎" else "secondary"):
                    st.session_state.feedback_map[fb_key] = "👎"
                    st.rerun()
...
with tab_campuses:
    ...
    ca, cb = st.columns(2)
    ca.info("🏙️ **Chamberí** — ideal if you value an urban, culturally immersive Madrid experience.")
    cb.info("🌿 **La Moraleja** — ideal if you prefer a quieter, greener campus just outside the city.")
    st.link_button("📅 Schedule a Visit", "https://www.brewstermadrid.com/admissions/visit-campus",
...
with tab_checklist:
    ...
    for i, (title, desc, link) in enumerate(CHECKLIST):
        ...
        with col_content:
            ...
            st.markdown(f'<div style="{style}"><b>Step {i+1}: {title}</b><br>'
                        f'<span style="font-size:0.85rem;color:#4a5568;">{desc}</span></div>',
                        unsafe_allow_html=True)
            if link:
                st.markdown(f"[→ {title}]({link})")
        st.markdown("")
    st.progress(len(done) / len(CHECKLIST),
                text=f"Progress: {len(done)}/{len(CHECKLIST)} steps complete")
    if len(done) == len(CHECKLIST):
        st.success("🎉 All steps complete! The Brewster Madrid team looks forward to welcoming your family.")
...
with tab_admin:
    ...
    if not log:
        ...
        c1.metric("Total Questions", len(log))
        ...
        for entry in reversed(log[-10:]):
            label = entry['user'][:80] + "…" if len(entry['user']) > 80 else entry['user']
            with st.expander(f"🙋 {label}"):
                st.markdown(f"**User:** {entry['user']}")
                reply = entry['assistant']
                st.markdown(f"**Assistant:** {reply[:300]}…" if len(reply) > 300 else f"**Assistant:** {reply}")
                st.markdown(f"**Feedback:** {entry.get('feedback','—')} &nbsp; **Time:** {entry['timestamp'][:19]}")
        st.markdown("---")
        csv_data = get_csv_download()
        if csv_data:
            st.download_button("⬇️ Download Full Log as CSV", data=csv_data,
